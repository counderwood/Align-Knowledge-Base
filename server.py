"""
Align Builders Knowledge Base — Cloud Server
=============================================
Connects to Google Drive, downloads PDFs automatically,
indexes them into ChromaDB, and serves the chat UI.

Deploy to Railway. Set these environment variables:
    ANTHROPIC_API_KEY   = your Anthropic key
    GOOGLE_CREDENTIALS  = contents of your JSON key file (paste the whole thing)
...
GOOGLE_CREDENTIALS = os.environ.get("GOOGLE_CREDENTIALS", "")  # full JSON string
...
SYNC_INTERVAL      = 3600  # re-check Drive every 60 minutes
...
def list_pdfs_in_folder(service, folder_id: str) -> list[dict]:
    """Return all PDFs in the Drive folder."""
...
def download_pdf(service, file_id: str) -> bytes:
    """Download a PDF from Drive and return as bytes."""
...
def extract_text(pdf_bytes: bytes, filename: str) -> str:
    ...
    if not text.strip():
        try:
            ...
            log.warning(f"pypdf also failed on {filename}: {e}")
...
_indexed_files = {}  # file_id → modifiedTime
...
def sync_drive():
    ...
    try:
        ...
        _sync_status["message"] = f"Found {len(new_or_updated)} new/updated PDFs to index..."
        ...
        for file in new_or_updated:
            try:
                ...
                if not raw_text:
                    log.warning(f"No text extracted from {file['name']} — may be scanned")
                    continue
        ...
        _sync_status = {
            "state": "done",
            "message": f"Ready — {total_chunks:,} chunks indexed from {len(_indexed_files)} PDFs",
            "indexed": indexed,
...
def background_sync_loop():
    """Run sync on startup, then every SYNC_INTERVAL seconds."""
    while True:
        sync_drive()
...
def manual_sync():
    """Trigger a manual re-sync from Drive."""
